...
### GDD 14–19 weapon/content architecture
...
- Power/weight/stability totals and overload ratio are computed.
- Overload currently penalizes spread, heat generation and reload time; exact balance remains tunable.
- Part effect IDs aggregate into a runtime effect list.
- Reusable `StatusController` supports stack limits, durations, periodic damage and movement/damage modifiers.
...
- Connector conventions now include matching connectors plus power_in/power_out and signal_in/signal_out pairs.

### Validation status
This environment has not run Godot. Runtime parsing, feel, device input, save integrity and performance are not validated.

### Major gaps
See `docs/GDD_COVERAGE.md`. Full part effect execution, final backpack UI/power routing, passive/active catalogs, characters, procedural map, zones, bosses, economy, curses, meta/hub, tutorial, UI/accessibility, production art/audio, save/stat/achievement/daily, Steam, endings/modes, content quantities, optimization and QA remain incomplete.

## Next Work — GDD coverage driven
1. Bind `WeaponBuild` computed stats/effects into `WeaponController` so assemblies change live combat.
2. Implement concrete effect execution: pellet, pierce, ricochet, explosive, beam, charge, elemental/status application.
3. Wire `StatusController` into enemies/player and projectile hit payloads.
4. Complete backpack connector geometry/power budgets and synergy activation rules.
...
- Next: bind these systems into actual combat behavior and expand effect execution.
