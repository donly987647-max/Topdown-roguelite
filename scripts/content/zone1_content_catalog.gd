...
func room_templates() -> Array[RoomTemplateDefinition]:
	return [
		...
		_room(&"z1_med_bay", &"rest", Vector2i(22,14), 0, 1, [], [], [Vector2i(10,4),Vector2i(10,9)], []),
...
func _enemy(id: StringName, title: String, cost: int, tags: PackedStringArray, scene_path: String, elite: bool, overrides: Dictionary) -> EnemySpawnProfile:
	var p := EnemySpawnProfile.new()
	p.id=id; p.display_name=title; p.threat_cost=cost; p.tags=tags; p.scene_path=scene_path; p.elite=elite; p.stat_overrides=overrides
	return p

func _room(id: StringName, type: StringName, size: Vector2i, threat: int, waves: int, spawns: Array[Vector2i], tags: PackedStringArray, obstacles: Array[Vector2i], hazards: Array[Vector2i]) -> RoomTemplateDefinition:
