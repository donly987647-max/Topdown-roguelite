...
func _physics_process(delta: float) -> void:
	var attack_speed_mult := _status.attack_speed_multiplier() if _status != null else 1.0
	_attack_cooldown_left = maxf(0.0, _attack_cooldown_left - delta * attack_speed_mult)
	...
	if _target == null or not is_instance_valid(_target) or _target.is_dead():
		...
		var desired := global_position.direction_to(_target.global_position) * move_speed * speed_mult
