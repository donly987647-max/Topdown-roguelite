...
func _ready() -> void:
	...
	weapon = ServicePistol.new()
	weapon.name = "ServicePistol"
	add_child(weapon)
	weapon.setup(self)
	weapon.ammo_state_changed.connect(_on_ammo_state_changed)
	EventBus.player_spawned.emit(self)
	_on_health_changed(health_component.get_snapshot())
	_on_ammo_state_changed(weapon.current_ammo, ServicePistol.MAGAZINE_CAPACITY, false)
	queue_redraw()
...
func _on_ammo_state_changed(current: int, capacity: int, reloading: bool) -> void:
	EventBus.ammo_changed.emit(current, capacity, reloading)
...
func _draw() -> void:
	...
	draw_line(Vector2.ZERO, aim_direction * 25.0, Color("ffbd55"), 3.0)
	draw_circle(aim_direction * 25.0, 3.0, Color("ffbd55"))
	if is_dash_invulnerable():
