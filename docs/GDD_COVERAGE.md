...
| 7 | 플레이어 조작 | **PARTIAL** | 키보드/마우스·Android 기초와 런타임 게임패드 이동/조준/사격/메뉴 포커스, Q/RB 장비 액티브, F/Y 캐릭터 액티브, I/D-pad Up 인벤토리 기반. 키 재지정·실기기 QA 미완 |
...
| 14 | 무기 시스템 | **PARTIAL** | InventoryPanel 4슬롯의 실제 WeaponBuild와 배치 모듈의 비누적 live modifier 재계산, v6 슬롯 저장/복원 기반. 12×4 전 조합·과부하 UX·밸런스/QA 미완 |
...
| 20 | 장비 격자 시스템 | **PARTIAL** | 실제 6×5 UI, stash↔grid/grid↔grid drag/drop, 배치/회전/자동배치/자동정렬 확인/되돌리기, 방향 단자·전력망, 직렬화와 전투 중 편집 잠금 연결. 정식 아이템 아트·전체 인접 규칙·수동 QA 미완 |
| 21 | 패시브 모듈 | **PARTIAL** | 배치/전력 상태가 실제 탄창·냉각·분산·넉백·치명타·폭발 반경에 반영되는 Zone1 6종 subset과 제거/회전 회귀 테스트. GDD 60종·조건부 트리거·밸런스/QA 미완 |
| 22 | 액티브 장비 | **PARTIAL** | 선택 active 1개, Q/RB 입력, 전력/쿨다운 HUD, 피격 취소 수리와 종료 과열 오버클럭 2종 및 v6 상태 저장/복원 구현. GDD 20종·연출·밸런스/수동 QA 미완 |
| 23 | 등급과 희귀도 | **PARTIAL** | 보상 rarity weighting/표시/경로 보너스 기반. 전체 드랍표·아트 규칙 미완 |
| 24 | 시너지 시스템 | **PARTIAL** | 방향성 네트워크가 실제 powered passive/active 실행을 차단·허용하고 회전 링크 회귀 테스트 통과. explicit/tag-tier 50종 실전 시너지 미완 |
...
| 31 | 제1구역 보스: 폐기물 압축기 GR-01 | **PARTIAL** | 전용 3페이즈, 압축벽/파편/컨베이어/낙하블록/추적톱날/잡병/안전구역/코어 노출과 필수 부품·키·확장/HP 선택 정산 기반. 정식 VFX/오디오/밸런스 QA 미완 |
...
| 47 | UI 구조 | **PARTIAL** | MainMenu/CharacterSelect/RunMap/Reward/Facility/CombatHUD/Inventory/Result와 캐릭터/장비 별도 active HUD, 패드 포커스 기반. 설정·최종 시각 체계 미완 |
| 48 | 인벤토리 UI | **PARTIAL** | 좌측 6×5/보관함과 pointer drag/drop, 우측 4 무기 슬롯/장비 active 지정/상세/네트워크, 회전·자동배치·빠른 이동·확인형 정렬·되돌리기·전투 잠금 구현. 전체 아이템 아트·수동 포인터/패드 QA 미완 |
...
| 57 | 저장 시스템 | **PARTIAL** | RunSaveService v6: 기존 전체 런/무기/가방 상태 + 선택 active item, instance별 cooldown/charge, 진행 중 장비 효과 atomic 저장·복원 회귀. 마이그레이션 fixture/손상복구/Steam Cloud 미완 |
