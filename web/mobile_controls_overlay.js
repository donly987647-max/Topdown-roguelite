(() => {
  ...
  style.textContent = `
    ...
    #lm-dom-controls { position: fixed; inset: 0; z-index: 2147483000; pointer-events: none; font-family: Arial, sans-serif; user-select: none; -webkit-user-select: none; }
  ...
  root.innerHTML = `
    ...
    <div id="lm-status">WEB DOM V5</div>
  `;
  document.body.appendChild(root);

  let touchCount = 0;
  const status = root.querySelector('#lm-status');
  const mark = () => { touchCount++; status.textContent = `WEB DOM V5 #${touchCount}`; };

  function bindPad(el, onValue, onRelease) {
    ...
    const updateGeometry = () => {
      ...
      cx = r.left + r.width / 2;
      cy = r.top + r.height / 2;
      radius = Math.max(1, r.width * 0.42);
    };
    const update = (e) => {
      updateGeometry();
  ...
  bindPad(root.querySelector('#lm-move'),
    (x,y) => { state.mx = x; state.my = y; },
    () => { state.mx = 0; state.my = 0; });
  bindPad(root.querySelector('#lm-aim'),
    (x,y,len) => { state.ax = x; state.ay = y; state.fire = len >= 0.12; },
    () => { state.ax = 0; state.ay = 0; state.fire = false; });
  ...
  bindButton('#lm-dodge', 'dodgeSeq');
  bindButton('#lm-reload', 'reloadSeq');
  bindButton('#lm-build', 'buildSeq');

  const clear = () => {
    state.mx = state.my = state.ax = state.ay = 0;
    state.fire = false;
  };
